...
namespace Space
{
	...
	void D3D11CommandList::SetIndexBuffer(IndexBuffer const* buffer)
	{
		...
		m_pDeferredContext->IASetIndexBuffer(
			pIndexBuffer->GetRawBuffer(), GetDXGIFormat(buffer->GetFormat()), (UINT)buffer->GetOffest());
	}
		...
		uint startSlot, uint numBuffers, VertexBuffer *const* buffer)
	{
		...
		for (auto iter = buffer; iter != buffer + numBuffers; iter++)
		{
			...
			striders.push_back((UINT)(*iter)->GetStride());
			offsets.push_back((UINT)(*iter)->GetOffest());
		}

		m_pDeferredContext->IASetVertexBuffers(
			startSlot, numBuffers, vbuffers.data(), (UINT*)striders.data(), (UINT*)offsets.data());
