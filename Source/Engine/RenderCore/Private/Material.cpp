...
namespace Space
{
	namespace Render {
		...
		Material::Material(tstring const& name)
		{
			...
			m_CommonVariablesBuffer = std::make_unique<ConstantBuffer>(
				sizeof(CommonVariables), (byte const*)&variables);
			...
			if (!tempFile.is_open())
			{
				throw std::exception("Missing Material Description File");
			}
			std::string json(
				...
				iter != nodeShaders.End(); ++iter)
			{
				...
				Shader* shader = nullptr; // new Shader(materialPath + toTs(nodeShaderName.GetString()));

				/*shader->SetConstantBuffer(0, m_CommonVariablesBuffer.get());
				shader->SetConstantBuffer(0, m_CommonVariablesBuffer.get());*/
			...
			_pipelineState = std::make_shared<PipelineState>(device);
		...
		Shader* Material::SelectShader()
		{
			m_CurrentShader = m_ShaderMap.at(m_ParameterSet);

			//if (nullptr != m_CurrentShader)
			//{
			//	byte const* signatureCode = m_CurrentShader->GetVertexShader()->GetByteCodes();
			//	size_t codeSizeInBytes = m_CurrentShader->GetVertexShader()->GetSizeInBytes();

			//	auto pInputLayout = InputLayout{
			...
			//	//_pipelineState->SetInputLayout(&pInputLayout, signatureCode, codeSizeInBytes);
			//}
		...
		void Material::Apply(CommandList* pCmdList)
		{
			auto currentShader = SelectShader();
			//m_pPipelineState->SetShaders(currentShader);
			//pCmdList->SetPipelineState(m_pPipelineState.get());
		}

		void Material::Update(CommandList* pCmdList)
		{
			...
			//m_CommonVariablesBuffer->UpdateToDevice(pCmdList);
		}
